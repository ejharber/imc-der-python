...
def clean_data(robot_data, mocap_data, ati_data, q0, qf):
    ...
    traj_robot = robot_data[500:1000, [0 ,2]]
    ...
    return traj_robot, traj_rope_base, traj_rope_tip, traj_force
...
def clean_raw_data():
    ...
    for file in os.listdir(file_path + folder_name):
        ...
        traj_robot, traj_rope_base, traj_rope_tip, traj_force = clean_data(data["ur5e_tool_data_save"], data["mocap_data_save"], data["ati_data_save"], data["q0_save"], data["qf_save"])

        # if np.any(traj_force > 100) or np.any(traj_force < -100): continue

        traj_robot_save.append(traj_robot)
    ...
    traj_robot_save = np.array(traj_robot_save)
    ...
    print(traj_robot_save.shape, traj_rope_base_save.shape, traj_rope_tip_save.shape, traj_force_save.shape, q0_save.shape, qf_save.shape)

    np.savez("filtered_data/N2.npz", traj_robot_save=traj_robot_save, traj_rope_base_save=traj_rope_base_save, traj_rope_tip_save=traj_rope_tip_save, 
