...
def interpolate_checkerboard_from_mocap(mocap_marker_data):
    ...
    x_1 = mocap_marker_data[np.logical_and(mocap_marker_data[:, 2] < z_mean, mocap_marker_data[:, 0] < x_mean), :] # bottom left
    x_2 = mocap_marker_data[np.logical_and(mocap_marker_data[:, 2] > z_mean, mocap_marker_data[:, 0] < x_mean), :] # top left
    x_3 = mocap_marker_data[np.logical_and(mocap_marker_data[:, 2] < z_mean, mocap_marker_data[:, 0] > x_mean), :] # bottom right
...
for file in os.listdir('data'):
    ...
    x_1 = mocap[np.logical_and(mocap[:, 2] < z_mean, mocap[:, 0] < x_mean), :]
    x_2 = mocap[np.logical_and(mocap[:, 2] > z_mean, mocap[:, 0] < x_mean), :]
    x_3 = mocap[np.logical_and(mocap[:, 2] < z_mean, mocap[:, 0] > x_mean), :]
