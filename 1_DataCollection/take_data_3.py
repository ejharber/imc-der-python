...
class UR5E(Node):
    def __init__(self):
        ...
        # ati cb
        self.ati_subscription = self.create_subscription(WrenchStamped, '/ati', self.ati_callback, 1, callback_group=client_cb_group)
        self.ati_data = None
        self.ati_data_zero = None

        # mocap cb
        self.mocap_subscription = self.create_subscription(RigidBodies, '/rigid_bodies', self.mocap_callback, 1, callback_group=client_cb_group)
        self.mocap_data = None

        # commanded cb
        self.ur5e_cmd_subscription = self.create_subscription(JointState, '/ur5e/command/joint_states', self.ur5e_cmd_callback, 1, callback_group=client_cb_group)
        self.ur5e_cmd_data = None

        # tool cb
        self.ur5e_tool_subscription = self.create_subscription(PoseStamped, '/ur5e/feedback/end_effector', self.ur5e_tool_callback, 1, callback_group=client_cb_group)
        self.ur5e_tool_data = None

        # joint measured cb
        self.ur5e_jointstate_subscription = self.create_subscription(JointState, '/ur5e/feedback/joint_states', self.ur5e_jointstate_callback, 1, callback_group=client_cb_group)
        ...
        timer_period = 1
        self.timer = self.create_timer(timer_period, self.timer_callback_command_robot, callback_group=timer_cb_group)
    ...
    def timer_callback_command_robot(self):
        ...
        for i in range(traj.shape[1]):
            ...
            # rclpy.time.sleep_for(rclpy.time.Duration(seconds=dt))
