...
class UR5e_CollectData(Node):
    def __init__(self):
        ...
        # mocap pb
        self.publisher_goal = self.create_publisher(PoseStamped, '/camera/goal', 1)
        self.mocap_data_goal = [-1.0, -1.0, -1.0]
        self.publisher_actual = self.create_publisher(PoseStamped, '/camera/actual', 1)
    ...
    def mocap_callback(self, msg):
        ...
        for rigid_body in msg.rigidbodies:
            ...
            self.mocap_data[0] = rigid_body.pose.position.x    
            self.mocap_data[1] = rigid_body.pose.position.y    
            self.mocap_data[2] = rigid_body.pose.position.z    

            self.mocap_data[3] = rigid_body.pose.orientation.w    
            self.mocap_data[4] = rigid_body.pose.orientation.x    
            self.mocap_data[5] = rigid_body.pose.orientation.y  
            self.mocap_data[6] = rigid_body.pose.orientation.z  
    ...
    def take_data(self):
        ...
        if len(self.mocap_data_goal) > 0:
            msg.pose.position.x = self.mocap_data_goal[0]
            msg.pose.position.y = self.mocap_data_goal[1]
            msg.pose.position.z = self.mocap_data_goal[2]
            self.publisher_goal.publish(msg)

        if len(self.mocap_data_actual) > 0:
            msg.pose.position.x = self.mocap_data_actual[0]
            msg.pose.position.y = self.mocap_data_actual[1]
            msg.pose.position.z = self.mocap_data_actual[2]
            self.publisher_actual.publish(msg)
